...
class Student {
	...
	addGrade(subject, score) {
		...
		if (!subjectKey) throw new Error("Mata pelajaran wajib diisi");
	...
	getAverage() {
		// Implementasi method di sini
		const gradeData = Object.values(this.#grades);
	...
	getGradeStatus() {
		// Implementasi method di sini
		const gradeCount = Object.keys(this.#grades).length;
		if (gradeCount === 0) return "Belum Ada Nilai";
	...
	displayInfo() {
		...
		const subjects = Object.keys(this.#grades).sort((a, b) =>
			a.localeCompare(b)
		);
